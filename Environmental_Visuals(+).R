...
elec_prod_per_source = data.frame("MWh" = c((1632/4565),(582/4565),(1197/4565),(742/4565),(410/4565),0,1/4565,
                                  ...
                                  "Type" = rep(c("Natural Gas","Coal", "Renewable Energy", "HydroElectrical",
                                                 "Network Produvtion","ISOZYGIO", "Else"),5),
...
graph2 = elec_prod_per_source %>%
  ...
  labs(title = "Contribution in Electricity Genration per Source")+
...
# Denmarks Average Temperature Dataset on how it evolves through 20th and 21th century
...
graph5 =
cool %>% 
  ggplot(aes(x = ï..Category, y = Annual.Mean))+
  geom_line(size = 0.8, colour = "red")+
  geom_point(aes(size = 1))+
  labs(title = "Average Temperature in Denmark", y = "Temperature(Celsius)", subtitle = "between 1901 - 2020")+
  theme_fivethirtyeight()+
...
kurac = read.csv(file.choose())

dansk_kurac = kurac[kurac$country == "Denmark",]# Specify Country
dansk_kurac = dansk_kurac[dansk_kurac$year >= 1990,]# Specify Time span
...
graph7 = 
  ...
  geom_point(aes(x = year, y = IncludingBiomass), size = 4)+
  ...
  scale_x_discrete(limits = seq(1990,2020,5))
